...
const Editor = () => {
  const { editorActions, clearActions, jumpToHighlight, pdfTitle, hasHighlights, setHasHighlights } = useNoteStore()
  ...
  useEffect(() => {
    ...
    (editorActions || []).forEach((action) => {
      ...
      if (action.type === 'INSERT_TITLE') {
        ...
        try {
            // Ensure we are focused at the end before inserting
            editor.commands.focus('end')
            
            if (editor.isActive('bulletList')) {
                editor.chain().splitListItem('listItem').insertContent(action.payload).run()
            } else {
                editor.chain().insertContent(`<ul><li>${action.payload}</li></ul>`).run()
            }
