"""App-wide design system — central tokens, widget builders, and the stylesheet.

Everything visual in iCharlotte should pull from here so the whole app looks
and behaves consistently. Nothing in this module imports other UI modules, so
it is safe to import from anywhere.

Usage:
    from icharlotte_core.ui import theme
    app.setStyleSheet(theme.app_stylesheet())         # once, on QApplication
    btn = theme.primary_button("Continue")
    title = theme.page_title("Summarize Documents")

History: this started life as the Wizard Mode design system
(icharlotte_core/ui/wizard/theme.py, now a re-export shim) and was promoted
app-wide. The stylesheet is applied at the QApplication level and cascades to
every widget; accent buttons created via the builders below carry their own
inline styling and intentionally override those defaults.
"""
...
def _asset_path(name: str) -> str:
    ...
    """
    return (Path(__file__).resolve().parent / "wizard" / "assets" / name).as_posix()
...
def style_tool_button(btn: QToolButton) -> QToolButton:
    """Secondary (outlined) look for QToolButton, e.g. dropdown menu buttons.

    Hides the menu indicator — callers using InstantPopup convey the dropdown
    with a "▾" in the text instead.
    """
    btn.setStyleSheet(
        f"QToolButton {{ background-color: #FFFFFF; color: {TEXT_BODY};"
        f" border: 1px solid {BORDER}; padding: 6px 16px; border-radius: {RADIUS_SM}px; }}"
        f"QToolButton:hover {{ background-color: {BG_SUBTLE}; border-color: #BCC4CC; }}"
        f"QToolButton:pressed {{ background-color: #EEF1F4; }}"
        f"QToolButton:disabled {{ color: {TEXT_FAINT}; background-color: #F5F6F8;"
        f" border-color: {BORDER_LIGHT}; }}"
...
def app_stylesheet() -> str:
    """QSS applied at the QApplication level so all default controls share a
...
def wizard_stylesheet() -> str:
    """Backwards-compatible alias — wizard roots predate the app-wide theme."""
