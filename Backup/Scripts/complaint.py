...
def call_gemini_api(prompt: str, context_text: str, models: list) -> Optional[str]:
    ...
    full_prompt = f"{prompt}\n\n[CONTEXT DOCUMENT]\n{context_text[:100000]}" # Limit context to avoid heavy over-token usage if massive
...
def save_case_state(file_num: str, key: str, value: Any):
...
def main():
    ...
    save_case_state(file_num, "file_path", case_path)
    ...
    if complaint_file:
        ...
        if complaint_text:
            save_case_state(file_num, "hidden_complaint_text", complaint_text)
    else:
    ...
    if complaint_text:
        ...
        """
        json_str = call_gemini_api(prompt_complaint, complaint_text, ["gemini-3-flash-preview", "gemini-2.5-flash"])
        if json_str:
            try:
    ...
    if insurance_text:
        ...
        Return ONLY the JSON.
        """
        json_str = call_gemini_api(prompt_ins, insurance_text, ["gemini-3-flash-preview", "gemini-2.5-flash"])
        if json_str:
            try:
                data = json.loads(clean_json_string(json_str))
                extracted_data.update(data)
    ...
    if caption_file:
        ...
        try:
            ...
            save_case_state(file_num, "caption", template_path)
            save_case_state(file_num, "caption_template_path", template_path)
        ...
        if caption_text:
            ...
            Return JSON: {"case_number": "found_number", "venue_county": "found_county", "client_name": "found_client_name"}
            """
            json_str = call_gemini_api(verify_prompt, caption_text, ["gemini-2.5-flash"])
            if json_str:
    ...
    if not extracted_data.get("case_number") and complaint_file:
        ...
        """
        json_str = call_gemini_api(recon_prompt, page1_text, ["gemini-2.5-flash"]) 
        if json_str:
             try:
                data = json.loads(clean_json_string(json_str))
    ...
    try:
        if complaint_text:
            ...
            if os.path.exists(FACTUAL_BG_PROMPT_FILE):
                ...
                fb_result = call_gemini_api(fb_prompt, complaint_text, ["gemini-3-flash-preview", "gemini-2.5-flash"])
                
                if fb_result:
                    save_case_state(file_num, "factual_background", fb_result)
