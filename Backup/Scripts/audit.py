...
CASE_DATA_DIR = os.path.join(os.getcwd(), ".gemini", "case_data")
...
def load_case_data(case_number):
    ...
    json_path = os.path.join(CASE_DATA_DIR, f"{case_number}.json")
    ...
    if os.path.exists(json_path):
        try:
            ...
            log_event(f"Error loading JSON: {e}", level="error")
            return None
    else:
        log_event(f"JSON data for {case_number} not found. Attempting to locate directory...")
        case_path = get_case_path(case_number)
        if case_path:
            log_event(f"Located case directory: {case_path}")
            data = {"file_path": case_path}
            save_case_state(case_number, "file_path", case_path)
        else:
