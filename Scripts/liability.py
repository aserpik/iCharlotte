...
def call_gemini(prompt, text):
    ...
    The model sequence comes from the Workbench-configurable ``agent_liability``
...
def add_markdown_to_doc(doc, content):
    ...
    while i < len(lines):
        ...
        # Specific formatting for "EVALUATION OF LIABILITY"
        if "EVALUATION OF LIABILITY" in stripped.upper():
...
def main():
    ...
    log_event(f"--- Starting Liability Agent for input: {input_path} ---")
    ...
    # Attempt to find file number in input path (####.###)
    file_num_match = re.search(r"(\d{4}\.\d{3})", input_path)
    if file_num_match:
        file_num = file_num_match.group(1)
        
        log_event(f"Saving liability analysis to case variable for {file_num}")
        # We manually tag it as "Liability" under "Reports" or "Analysis"
        # Based on rules, "Reports" seems best fit or we can add "Analysis" tag if rules allow.
        # Let's use "Reports" -> "Liability" if supported, or just extra tags.
        # Current rules have "Reports" top level.
        data_manager.save_variable(file_num, "liability_evaluation", summary, source="liability_agent", extra_tags=["Reports", "Liability"])
    else:
    ...
    output_filename = "liability_eval.docx"
