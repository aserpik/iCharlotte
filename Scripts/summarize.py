...
def main():
    ...
    # 3b. Save to Case Data (if file number found)
    data_manager = CaseDataManager()
    
    # Simple extraction logic: find ####.### pattern in path
    ...
    if file_num_match:
        ...
        # Clean variable name (replace spaces with underscores, alphanumeric only)
        clean_var_name = re.sub(r"[^a-zA-Z0-9_]", "_", base_name.lower())
        var_key = f"summary_{clean_var_name}"
        
        log_event(f"Saving summary to case variable: {var_key} for {file_num}")
